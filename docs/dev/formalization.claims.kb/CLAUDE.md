...
Read `a-structure-earns-its-keep-by-the-decision-it-changes.md` first. Most
claims are its consequences; three are its unstated premises, filed after the
fact because the bar it states cannot grade them -- `CARRIER` (the choice of
objects, which no count refutes), `RETRIEVAL` (whether the claim reaches the
decision at all), and `REMIT` (whether the job is covered, which no audit of
filed claims can see).

## Cheapest order of operations

The order that produced the ledger above, with the mistakes taken out.

Before step 1, two statements no later step can check: name the objects
(`CARRIER`) -- the census is a census *of* them -- and state the job
(`REMIT`) -- the decisions the theory must govern, which fixes what coverage
means. Spend real effort on both and expect the expensive error here.
...
7. **When the claims feel done, grade coverage against the job statement.** The
   checks catch broken terms, never missing ones (`REMIT`); the audit that only
   reads what exists converges on defects in what exists.
